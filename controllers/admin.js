...
const adminLogin = TryCatch(async (req, res, next) => {
  ...
  const adminSecretKey = process.env.ADMIN_SECRET_KEY || "syamkola";
  ...
  try {
    ...
    const transformedMessages = messages.map((message) => {
      ...
      return {
        ...
        createdAt,
        chat: chat ? chat._id : null,
        groupChat: chat?.groupChat || null, // Ensure 'groupChat' is derived correctly
        sender: sender
          ? {
              _id: sender._id,
              name: sender.name,
    ...
    console.error("Error in allMessages:", error); // Log the error for debugging
    return res.status(500).json({
      success: false,
      message: error.message,
    });
